 ...
 * communicates time changes to its clients. The text version of the class does not maintain a
 * clock, since the game is turn based.
 *
 * @author rylanl
 */
public class TextUpdater extends Updater {

  PriorityQueue<Updatable> theUpdateQueue;
  GameState theGameState;
  boolean isPaused = true;

  public void handle(double deltaT) {
  ...
  // determine order
  public TextUpdater(GameState aGameState) {
  ...
  private void checkForGameEnd() {
    ...
    if (theGameState.isDone()) {
      TextScreen theScreen = (TextScreen) theGameState.getScreen();
      int score = theGameState.getScore();
      if (theGameState.isWon()) theScreen.displayWinMessage(score);
      else if (theGameState.isLost()) theScreen.displayLooseMessage(score);
      else theScreen.displayQuitMessage(score);
      System.exit(0);
    }
  }
}
